...
### JMM
JMM定义了并发编程的规范，保证多线程情况下对于线程共享变量的可见性。其中最重要的是happens-before原则，如果一个操作A happens-before操作B，那么A的结果对于B而言一定是可见的，即使是在多线程的情况下。比如volatile变量的happens-before原则：volatile修饰的变量的写操作happens-before读操作，那么写操作对于其之后的读操作一定是可见的。还有一些额外的happens-before原则：
...
#### synchronized锁升级
对象的锁信息存放在对象头中的Markword中，Markword中的信息根据锁状态不同会发生变化：
![](./static/ObjectHeader.png)
jdk1.6以后，synchronized有四种状态：
1.无锁：对共享资源不加锁，此时Markword中的信息为对象HashCode，GC年龄，锁标志(01表示无锁)

2.偏向锁，线程访问无锁状态的对象时，会将线程id记录在Markword中，同时将偏向锁标识位置为1。偏向锁的意义在于，**在没有偏向锁的情况下，如果只有轻量级锁，那么每次都会进行cas替换，在只有一个线程的情况下会带来多余的开销；有了偏向锁，只需比较偏向线程是否是当前线程，如果是则直接获取，无需cas加锁**
如果持有偏向锁的线程离开了同步块，那么此时有另外的线程竞争时，会重新偏向，将偏向id设置为当前竞争的线程

3.轻量级锁，当线程尝试获取偏向锁对象时，此时偏向锁会升级为轻量级锁。此时线程栈中会生成一个锁记录Lock Record，
![](./static/lightlyLock0.png)
锁记录中 Object reference 指向锁对象，并尝试用 cas 替换 Object 的 Mark Word，将 Mark Word 的值存入锁记录，然后将锁记录的信息存到Object的Mark Word中,如果替换成功，那么线程获取到锁，可以执行同步代码块。
![](./static/lightlyLock1.png)
![](./static/lightlyLock2.png)
如果cas失败，那么有两种情况：
一、如果是自己执行了synchronized锁重入，那么会在线程栈中再加一条Lock Record作为可重入计数，此时对象头中的Object Markword为null
![](./static/lightlyLock3.png)
二、其他线程持有轻量级锁，此时会进行锁升级

当退出 synchronized 代码块（解锁时）如果有取值为 null 的锁记录，表示有重入，这时重置锁记录，表示重入计数减一；锁记录的值不为 null，这时使用 cas 将 Mark Word 的值恢复给对象头：成功，则解锁成功；失败，说明此时进行了锁升级，进入重量级锁解锁流程。

当轻量级锁是由偏向锁转化来的时候，持有偏向锁的线程会加一条Lock Record，并将Markword写入Lock Record

4.重量级锁，当轻量级锁自旋超过一定次数后，锁会升级为重量级锁。此时会给锁对象指定一个monitor，Object的Markword指向monitor锁的地址。monitor的owner字段设置为当前持有锁的线程，之后的线程会进入monitor的EntryList处于Blocked状态，当持有锁的线程解锁时，按照 Monitor 地址找到 Monitor 对象，设置 Owner 为 null，唤醒 EntryList 中 BLOCKED 线程。

当重量级锁是由轻量级锁升级而来，此时线程T0持有轻量级锁，当T0退出同步块解锁时，使用 cas 将 Mark Word 的值恢复给对象头，会失败，这时会进入重量级解锁流程
![](./static/heavyLock.png)
### ReentrantLock
...
### 不同线程池的使用场景
‌NewCachedThreadPool‌：适用于执行大量短期异步的小程序或负载较轻的服务器。它根据需要创建新的线程，并且可以灵活地回收空闲线程，没有核心线程的概念，最大线程数理论上没有限制。使用SynchronousQueue作为任务队列，当提交新任务时，若没有空闲线程可用，则会创建新的线程执行任务。适合处理突发的、高并发的请求，或者执行任务量大但耗时较短的任务。

‌NewFixedThreadPool‌：适用于执行长期任务，性能较好。它创建一个固定大小的线程池，核心线程数和最大线程数相等，设置明确的线程数量，一旦创建就不会再增加或减少。配合有界任务队列（通常是LinkedBlockingQueue），当线程池满时，新提交的任务将等待直到有线程空闲出来。适合资源有限或系统对资源消耗有严格要求的情况，或者需要控制并发数，防止过多的线程导致系统负载过重。

‌NewSingleThreadExecutor‌：适用于需要保证顺序执行的场景，例如单线程处理任务。它创建一个只有一个线程的线程池，且线程的存活时间是无限的。当该线程忙碌时，新任务会进入阻塞队列中等待执行。适用于需要保证任务按顺序执行的场景，或者希望所有任务都在同一个线程中依次执行的情况。

‌ScheduledThreadPool‌：适用于需要执行定时或周期性任务的场景。它允许你安排定期执行的任务，适用于需要定期执行某些操作的应用程序，如数据库清理工作等。通过ScheduledExecutorService来安排这些任务，可以灵活地设置任务的执行时间和频率。

### 线程池的不同关闭方法
1、shutDown()方法，shutDown方法会等待线程池以及阻塞队列中的任务都执行完毕之后再关闭线程池
2、shutDownNow()方法，该方法会强制关闭线程池，尝试中断正在执行的任务，并返回等待执行的任务列表。它会立即停止线程池的运行，不再处理等待队列中的任务。
3、waitTermination()方法，该方法会等待线程池内的所有任务执行完毕，或者在指定的时间内等待线程池任务执行完毕。它会阻塞当前线程，除非：一、线程池内所有任务执行完毕；二、线程池任务执行时间超过了指定的时间；三、当前线程中断
### AQS
...
#### 资源不足的情况
1、堆内存本身分配不够
排查：使用命令jmap -heap命令，查看java堆内存使用情况，看是否分配内存太小，再进行相应调整

#### 不当操作的情况
2、一次性申请的堆内存太多
比如数据库里有千万级的数据，如果一次性全部读入，势必会造成OOM，这种情况就需要分批读取

3、内存资源耗尽没有释放
比如通过jdbc连接数据库，如果大量线程都连接到数据库而没有释放，连接数多了以后就会造成OOM，这种情况需要及时释放不再使用的资源

针对以上两种情况，可以通过java中的dump文件进行排查进行错误代码的快速定位，dump文件记录了堆信息，dump文件可以通过以下的设置生成：
①、提前设置JVM参数：-XX：+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath：xxx，这个参数的意思是当发生OOM时在HeapDumpPath下生成对应的dump文件
②、通过jmap -dump命令在运行时导出dump文件，但是会导致STW

获得dump文件后，再通过java VisualVM软件可视化dump文件，查找可疑的业务对象，并查找其GCRoot，进行相应的代码分析

### 频繁minorGC/fullGC怎么办
通过jstat -gc命令查看gc情况
#### 频繁minorGC
![alt text](image-12.png)
YGC为minorGC的耗时，YGCT为minorGC的时间间隔，如果时间间隔太短说明minorGC频繁
通常情况下对象会在年轻代被minorGC回收掉，如果频繁minorGC，那么可能是因为年轻代的空间太小，此时可以通过fullGC观察对象有没有被回收掉，因为如果是因为空间太小，那么对象会进入到老年代中，如果fullGC能回收掉，那么就印证了是年轻代空间太小的原因，此时可以调整年轻代的空间大小

#### 频繁fullGC
可能的情况：
1、不断有大对象进行老年代
2、内存泄漏，导致年轻代回收不掉，进入老年代也回收不掉
3、程序频繁生成一些长生命周期的对象，导致持续进入老年代把老年代空间挤满
这种情况可以先通过jstat -gc查看老年代内存使用情况，如果确实内存使用率比较高，可以1、调整堆内存大小 2、导出dump文件对占用内存高的业务对象进行分析，跟oom时的分析一样，看gcroot定位代码
...
### 对象一定分配在堆上吗
不一定，如果对象是在方法中新建的局部变量，并且没有返回，那么此时对象是在栈中分配的内存；
反之，如果返回了局部创建的对象，那么此时就发生了**方法逃逸**，此时的对象是在堆上进行内存分配的
...
### 事务失效
事务实现分为编程式事务与声明式事务，编程式事务是通过编码的方式来实现事务，而声明式事务是通过注解的方式来实现事务。
...
1、写入数据时候先查表中有无该数据，如果有则不写入
2、通过加版本号version的方式，在写入数据库时同时更新version，如果更新成功则插入，否则失败
3、加一个消息处理表，在消息来之后先查表看有没有消费过，如果消费过就丢弃
...
2.提高数据传输的效率。接收窗口可以使发送方知道哪些消息被接受，哪些消息没有被接受，对于没有被接受的消息会启动超时重传，并且滑动窗口需要保证最前面的消息接收到之后才滑动，因此保证了消息的有序性。如果没有滑动窗口，那么后面的消息需要前面的消息得到确认以后才能发送，拖慢了数据发送的效率
### 为什么需要time-wait
...
## HTTP
### http1.1相对http1.0改进了什么
1、支持长连接
2、使用管道传输，可以不等请求的回复便发送下一次请求
### http2相对于http1.1改进了什么
1、进行了头部压缩，相同或者类似的header会消除相同的部分。实现基础是HPACK算法，服务端和客户端同时维护了一张头信息表，所有字段都会存入这个表并生成一个索引号，这样当遇到相同的字段时只需要发送索引号
2、http2相比与http1.1的纯文本报文，头信息和数据体都采用二进制传输，计算机无需再进行转换，加速了数据传输的效率
3、引入了stream机制，可以并行的传输多个请求，解决了http1.1中队头阻塞的问题。这里的队头阻塞指的是http1.1中无法对请求分块解析，只有一个请求处理完成后才能处理下一个请求
4、服务端可以主动向客户端推送消息。通过分配给服务端和客户端不同奇偶的stream序号，可以使服务端能够主动向客户端推送消息

### http3.0相对于http2.0改进了什么
http2的传输层协议是基于TCP协议的，因此达到服务端的字节序号必须有序，当【前一个字节】没有到达时，后面的字节就算到了也只能缓存在内核里，因此可能会导致前面的包丢失了使得后面的包不得不等待其重传，造成队头阻塞
http3.0将传输层协议由TCP换成了基于UDP的QUIC协议，由于UDP无需有序确认，因此不存在队头阻塞的问题。
1、不存在队头阻塞，http3.0也有类似stream的概念，但是当一个stream发生丢失时，只会影响当前stream，而不会影响其他stream
2、更快的连接建立，QUIC协议里包含了TLS协议，因此无需像http2.0一样需要先进行TCP连接，再进行TLS连接，而是只需要经过一次连接号的确认即可，耗时1RTT
3、连接迁移，QUIC协议没有像TCP协议一样选择四元组标记连接，而是客户端和服务端各自选择一组连接ID标识自身，这样即使IP地址发生变化，但只要连接上下文（连接ID，TLS密钥等）没有变化，就可以直接复用连接，而无需再连接
...
5xx表示服务端异常，501 Not Implemented表示服务端暂时还未实现该功能，502 Bad Gateway表示服务器工作正常，但是收到了错误的请求，503 Service Unavailable 表示服务器当前忙,504 Gateway Timeout表示服务器工作正常，但是访问服务器出了问题，但是服务器没有及时响应
...
## mysql的优化思路
1、前缀索引优化，对记录的几个前缀建立索引，减少索引字段的大小，使一个索引页面能够容纳下更多的索引项
2、覆盖索引优化，使查询中的字段在二级索引中都能找得到，避免回表
3、主键索引最好自增，避免插入中间的键值引起页面的分裂
4、主键不应太大，否则其他的索引也会很大
5、对于需要经常排序的字段，最好对其建立索引，利用mysql本身对其排序
6、索引最好设置为NOT NULL
7、大数据量查询时limit优化，最好加上无序查询，这样可以避免数据库排序所耗费的时间

## 索引建立的原则
1、建立索引的字段区分度要高
2、针对需要经常排序的字段建立索引
3、使用前缀建立索引，一方面压缩存储空间，另一方面能够使一页索引页可以存储下更多的索引
4、尽量建立联合索引
...
## 为什么用RPC不用HTTP
HTTP中对于服务调用来说有太多冗余信息，比如头部规范的字段等等，这些冗余信息会拖慢传输的速度，RPC可以基于TCP自定义传输格式，可以只关注自己想要的数据。除此之外，一些好的RPC框架还支持自动服务注册与发现，智能负载均衡等
...
![](./static/MESI_07.png)
常说的指令重排和cpu乱序执行**有时候**(还有编译器优化的原因)就是因为这两个队列的原因，比如说，有两条语句，先修改D，再修改C。但是C先收到ack，D后收到ack，这样看起来就像是C和D执行顺序颠倒了一样
![](./static/MESI_08.png)
![](./static/MESI_09.png)
**那么，什么是内存屏障？**
...
## 挑一个你觉得最难的项目讲一下吧
比较难的项目我觉得有两个，一个是我在实习期间做的一个需求叫站长工作台。这个需求的背景是提供给网格站站长一些方便他们管理的信息，比如网格站下有哪些司机的配送任务要超时了；有哪些取回任务还没有进行处理等等，用来提升他们的工作管理效率。这个需求的难点就在于需要查询的数据量巨大，就拿配送单来说，每天全国产生的配送单数是以万计算的，而要查询的配送单范围可能是以周为单位，或者说以月为单位，所以数据量可想而知。而上游服务要求的查询性能又比较高，单体服务的查询时间最好要控制在100ms以内，但是在这么大量的配送单面前，如果直接查肯定是会造成慢查询的。事实上在加了索引的情况下，直接查扫描的行数还是会有10000行以上，性能是肯定满足不了要求的。因为和配送单关联的有一个单据叫运单，一个运单是对应多个配送单的，而运单的状态又和配送单关联，所以最后我想到的解决方法是先查指定状态的运单，再通过运单查制定状态的配送单。这样相当于是在业务层面对单据过滤了一遍，而不是直接交给数据库。最后测试的时候也发现通过这样查的方式，运单和配送单的查询行数都只有几百行，比直接查性能提升了很多。